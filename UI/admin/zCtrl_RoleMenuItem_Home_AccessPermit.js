...
.controller("roleMenuItemCtrl", ['$scope', '$rootScope', '$resource', '$window', '$location', '$route', '$routeParams', 'zSrv_OAuth2', 'zSrv_ResourceServer', 'zSrv_InputCustom', 'uiGridConstants', 'zSrv_MagnificPopUp',
...
//            rolesMenuItem: '/roleMenuItemAccessGrants',
        ...
        $scope.group.updateRolesMenuItemModel = function () {
            ...
            for ( m in arr) { // for each menuItemId
                for (r in arr[m]) {  // for each roleId
                        for (a in arr[m][r]) {
                            var changeData = {
                                menuItemId: m,
                                roleId: r,
        ...
        $scope.group.getRolesMenuItemModel = function () {
            ...
            modelResource.get().$promise.then(function (resultModel) {
                ...
                for (m in grp.zModel)   // list of menuItems
                    for (r in grp.modalOptionsCollection) { // list of roles 
                ...
                //build Full Set of index table for menuItemId + roleId => accessGrants recordset refer to each menuItemId
                //var roleMenuItemSet2 = [];
                //var arr = roleMenuItemSet2;
                for (r in grp.roleMenuItemSet) {
                    var accessGrant = {
        ...
        $scope.group.openAccessGrantsModal = function (ev, rowEntity) {
            ...
            mg.showModal($scope, ev, view, false).then(function (selectedEntity) {
                ...
                for (s in selectedEntity.AccessGrants) {
            ...
            //console.log('group.modalOptionsCollection = ' + JSON.stringify( $scope.group.modalOptionsCollection));
            //console.log('rowEntity.AccessGrants = ' + JSON.stringify(rowEntity.AccessGrants));
            //console.log('group.ngDropDownMultiselectConfig = ' + JSON.stringify($scope.group.ngDropDownMultiselectConfig))
            //var modalInstance = $uibModal.open({
            //    animation: true,
            //    controller: 'accessGrantModalCtrl',
            //    template: '<div class="modal-header">' +
            //                    '<h3 class="modal-title">Access Grants Authorization</h3>' +
            //                '</div>' +
            //                '<div class="modal-body">' + 
            //                    '<div ng-dropdown-multiselect="" options="group.accessGrantsOptionsCollection" selected-model="rowEntity.AccessGrants" extra-settings="group.ngDropDownMultiselectConfig"></div>' +
            //                '</div>' +
