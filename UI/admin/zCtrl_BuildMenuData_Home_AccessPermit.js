...
.controller("buildMenuDataCtrl", ['$scope', '$rootScope', '$resource', '$q', '$window', '$location', '$route', '$routeParams', 'zSrv_OAuth2', 'zSrv_ResourceServer', 'zSrv_InputCustom', 'uiGridConstants', 'zSrv_Field',
    function ($scope, $rootScope, $resource, $q, $window, $location, $route, $routeParams, zSrv_OAuth2, zSrv_ResourceServer, zSrv_InputCustom, uiGridConstants, zSrv_Field) {
        ...
        $scope.$on("$routeChangeSuccess", function () {
            //if ($location.path().indexOf("/buildMenuData") == 0) {
            //    var id = $routeParams["id"];
            //    $scope.group.zModel = {};
            //    $scope.group.formHeader = $scope.group.name + " - List";
            ...
            //}
            //var f = zSrv_InputCustom.formFields({ name: 'buildMenuData' });
            ...
            for (elem in details) {
                //if (elem <= 17 && elem > 4) {
                   // if (elem == 17) {
                    ...
                    //        enableSorting: null, //details[elem].enableSorting,
        ...
        grp.importFieldElement = function () {
            ...
            ////+++++++++++++++++
            //grp.name = 'ALL';
            //masterFields = zSrv_InputCustom.formButtons(grp);
            //for (f in masterFields) {
            //    var fields = [];
            //    var master = { Name: f, Type: 'FormButton', Elements: [] };
            //    fields.push(master);
            //    _fillElements(master.Elements, masterFields[f]);
            //    var deferred = $q.defer();
            //    promiseArray.push({ deferred: deferred, promise: deferred.promise, data: fields });
            ...
            ////+++++++++++++++++
            //grp.name = 'ALL';
            //masterFields = zSrv_InputCustom.listButtonTop(grp);
            //for (f in masterFields) {
            //    var fields = [];
            //    var master = { Name: f, Type: 'ListButtonTop', Elements: [] };
            //    fields.push(master);
            //    _fillElements(master.Elements, masterFields[f]);
            //    var deferred = $q.defer();
            //    promiseArray.push({ deferred: deferred, promise: deferred.promise, data: fields });
            ...
            ////+++++++++++++++++
            //grp.name = 'ALL';
            //masterFields = zSrv_InputCustom.listButtons(grp);
            //for (f in masterFields) {
            //    var fields = [];
            //    var master = { Name: f, Type: 'ListButton', Elements: [] };
            //    fields.push(master);
            //    _fillElements(master.Elements, masterFields[f]);
            //    var deferred = $q.defer();
            //    promiseArray.push({ deferred: deferred, promise: deferred.promise, data: fields });
            ...
            for (i = 0; i < promiseArray.length - 1; i++) {
                (function () {
                    var index = i;
