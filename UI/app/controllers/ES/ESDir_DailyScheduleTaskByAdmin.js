...
angular.module("zDir_DailyScheduleAdmin", [])
.directive("dailyScheduleAdmin", ['$compile', '$rootScope', '$q', 'zSrv_OAuth2', 'zSrv_ResourceServer', 'zSrv_InputCustom', '$location', 'dataService', '$http',
        ...
        link: function (scope, element, attrs) {
            ...
            var getApp = function (slot, consultant, interval, status) {
                ...
                for (app in scope.gridData) {
                    ...
                    if (status == "CONFIRMED") {
                        if (scope.gridData[app].AssignToConsultantId == consultant && scope.gridData[app].BookOn >= startTime && scope.gridData[app].BookOn < stopTime && scope.gridData[app].Status == "2") {
                        ...
                        if (scope.gridData[app].AssignToConsultantId == consultant && scope.gridData[app].BookOn >= startTime && scope.gridData[app].BookOn < stopTime && scope.gridData[app].Status == "3") {
                            ...
                            var srcGender = "";
                            ismale = false;
                            if (gender == "F") srcGender = "Templates/images/icon-male.png";
                            ...
                            apps.push({ custId: scope.gridData[app].CustomerId, custId_Aes: scope.gridData[app].CustomerId_AES, title: title, slot: bookslot, consultant: scope.gridData[app].AssignToConsultantId, appid: scope.gridData[app].Id, gender: scope.gridData[app].Gender, srcviptype: srcVipType, isviptype: isviptype, srcgender: srcGender, ismale: ismale, isscan: isscan, srcscan: srcScan, isskinanalysis: isskinanalysis, srcskinanalysis: srcSkinAnalysis, isanotherconsultant: isanotherconsultant });
                        }
                    }
                    else if (status == "CANCEL") {


                        if (scope.gridData[app].AssignToConsultantId == consultant && scope.gridData[app].BookOn >= startTime && scope.gridData[app].BookOn < stopTime && scope.gridData[app].Status == "4") {
                ...
                $q.all([defer_column.promise, defer_general.promise, defer_weekday.promise, defer_prefer.promose, defer_data.promise, defer_blockslot.promise, defer_outlet.promise, defer_vipcategory.promise, defer_display.promise]).then(function () {
                    ...
                    for (var i = 1; i <= rows; i++) {
                        ...
                        for (var j = 0; j < cols; j++) {
                            ...
                            var appcan = getApp(scope.rowHeaderData[i - 1], scope.columnHeader[j].Id, interval, "CANCEL");
