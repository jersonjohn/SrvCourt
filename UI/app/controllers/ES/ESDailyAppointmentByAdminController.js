'use strict';
//eApptDailyAppointmentByAdminController
define(['app'], function (app) {
    ...
    var zc = function ($scope, $http, $resource, $window, $location, $route, $routeParams, zSrv_ResourceServer, zSrv_InputCustom,
        ...
        $scope.checkingCustomerId = false;

        $scope.consultanttotal = [];
        $scope.slottotal = [];
	  
	   $scope.AssignedToConsultantId ="";
        ...
        $scope.group = {
            ...
            formHeader: 'Outlet Daily Schedule-Admin',

            name: 'eApptDailyAppointmentByAdmin',
        ...
        $scope.$on("$routeChangeSuccess", function () {
            ...
            zSrv_MasterData.loadLoginData().then(function () {
                ...
                $scope.brands = $scope.masterData.brands;
        ...
        $scope.OnChangeOutlet = function () {
            ...
            if ($scope.isLoadingBrand == true) return;
        ...
        $scope.OnChangeBrand = function () {
            ...
            $scope.loadMasterData = false;
            $scope.isLoadingData = true;
            zSrv_InputCustom.httpGet(zSrv_ResourceServer.getURL('eApptMtrOutletUrl'), { "referId": brandId }).then(function (respOutlets) {
                $scope.outlets = respOutlets;

                $scope.outletId = $scope.outlets[0].Id;
                $scope.outletName = $scope.outlets[0].Name;

                zSrv_MasterData.loadMasterData(brandId, $scope.outletId).then(function () {
                    ...
                    $scope.accessSetting = $scope.masterData.AccessSetting;
                        ...
                        //$scope.brandId = $scope.brand.BrandId;
                        $scope.brandName = $scope.brand.Name;
                        $scope.brandShortName = $scope.brand.ShortName;
        ...
        $scope.OnChangeCustomerId = function () {
		   ...
		   if ($scope.checkingCustomerId == true) {
                console.log('already processing');
                return;
            }

            $scope.checkingCustomerId = true;
            ...
            zSrv_InputCustom.httpGet(resourceServerUrl + 'api/getCustomerByAesId/' + Customer_AesId).then(function (respCustomer) {
                ...
                if ($scope.CustomerDetails != null && $scope.CustomerDetails.length > 0) {
                    ...
                    //$scope.modalGroup.zModal.AssignToConsultantId
                    if ($scope.CustomerDetails[0].ConsultantId != $scope.AssignedToConsultantId) {

                        $scope.message = "Customer belongs to another Consultant(" + $scope.CustomerDetails[0].ConsultantId + ") in eAppt";
                    }

                    if ($scope.CustomerDetails[0].OutletId != $scope.outletId) {

                       $scope.message = "Customer belongs to another Outlet(" + $scope.CustomerDetails[0].OutletId + ") in eAppt";
                    }
                    ...
                    var AesWebApi = "";
                    var country = $rootScope.MyProfile.Country;
                    if (country == "SG") {
                        AesWebApi = zSrv_ResourceServer.getURL('eApptSGAesCustomerUrl') + "api/" + $scope.brandShortName + "/Customer/fnGetCustomerDetails";
                    }
                    else if (country == "MY") {
                        AesWebApi = zSrv_ResourceServer.getURL('eApptMYAesCustomerUrl') + "api/" + $scope.brandShortName + "/Customer/fnGetCustomerDetails";
                    }
                    else {
                        AesWebApi = zSrv_ResourceServer.getURL('eApptSGAesCustomerUrl') + "api/" + $scope.brandShortName + "/Customer/fnGetCustomerDetails";
                    }
                    ...
                    zSrv_InputCustom.httpPost(AesWebApi, customer).then(function (respAesCustomer) {
                        ...
                        if ($scope.AesCustomerDetails.isExist == false) {
                            ...
                            //$scope.modalGroup.zModal.AssignToConsultantId
                            if ($scope.AesCustomerDetails.customer.objbeCustomerConsultant.Staff_ID != $scope.AssignedToConsultantId) {
                                $scope.message = "Customer belongs to another Consultant(" +  $scope.AesCustomerDetails.customer.objbeCustomerConsultant.Staff_ID + ") in aes";
                            }

                            if ($scope.outletName != customerBranch) {
                               $scope.message = "Customer belongs to another Outlet(" + customerBranch + ") in aes";
                            }
                            ...
                            zSrv_InputCustom.httpGet(zSrv_ResourceServer.getURL('eApptMtrConsultantUrl'), { "ConsultantId_AES": ConsultantId_AES,"OutletId": $scope.outletId }).then(function (consultant) {
                                ...
                                if (consultant[0].Id != null || consultant[0].Id != "" || typeof consultant[0].Id == "undefined") {
								    ...
								    var gender = $scope.AesCustomerDetails.customer.Gender;
                                    if (gender == "MALE") {
                                        gender = "M";
                                    }
								    ...
								    //customer.LastVisit = $scope.AesCustomerDetails.customer.LastVisit_Date;
								  
								    if ($scope.AesCustomerDetails.customer.LastVisit_Date != null && $scope.AesCustomerDetails.customer.LastVisit_Date != ''){
								      customer.LastVisit = $scope.AesCustomerDetails.customer.LastVisit_Date;
                                    }
                                    else{
                                      customer.LastVisit = new Date(1999, 12, 31);
                                    }
        ...
        var IsCustomerExistsInSlot = function (row, col, customerId) {
            var IsExists = false;
        ...
        $scope.callAddAppointmentPopupNew = function (ev, customerId, customerName, appid, consultantId, date, time, row, col) {
           ...
           $scope.AssignedToConsultantId = consultantId;
        ...
        $scope.differentOutlet = function () {
         ...
         $scope.printPage = function (imageFile) {
          ...
          $scope.searchFromUpdate = function () {

            //$scope.modalGroup.zModal.CustomerId_AES = customerId_Aes;
            $scope.message = "";
            console.log('popup called');
            $scope.modalGroup = zSrv_MagnificPopUp;
            var mg = $scope.modalGroup;
    ...
    app.register.controller('eApptDailyAppointmentByAdminController', zc);
