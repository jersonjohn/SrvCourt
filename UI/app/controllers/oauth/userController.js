...
define(['app'], function (app) {
    ...
    var userController = function ($scope, $rootScope, $resource, $window, $location, $route, $routeParams, zSrv_OAuth2, zSrv_ResourceServer, zSrv_InputCustom, uiGridConstants,
                        zSrv_MagnificPopUp, Upload, zSrv_zNotify, zSrv_Field) {
        ...
        $scope.$on("$locationChangeStart", function (event) { zSrv_InputCustom.locationChangeStart(event, $scope.group.ngScope) });
        ...
        $scope.group.deleteModel = function (user) {
            ...
            zSrv_MagnificPopUp.showConfirm(ev, 'Delete Confirmation', 'Are you sure?').then(function () {
                ...
                zSrv_OAuth2.deleteUser(user.UserName).then(function (result) {
                    ...
                    zSrv_zNotify.note('success', 'Record Deleted', user.UserName + ' is removed successfully.');
