...
define(['app'], function (app) {
    ...
    var zc = function ($rootScope,$scope, $http, $resource, $window, $location, $route, $routeParams, zSrv_ResourceServer, zSrv_InputCustom,
        ...
        $scope.$on("$routeChangeSuccess", function() {
            ...
            routeInit.then(function () {
                ...
                if (grp.ngLocation.path().indexOf(grp.createModelURL) >= 0) {
				    ...
				    console.log("Default listing all tickets created by the User "+$rootScope.MyProfile.UserName);
        ...
        grp.StatusOnChange = function(){
            var rec = findRecordById(grp.zData.Status, grp.zModel.StatusVal.Id);
		    console.log("Status onChange event Fired : LIST :"+JSON.stringify(grp.zData.Status)+" REC:"+JSON.stringify(rec));
            if (rec) {
                grp.zModel.Status = rec.SortId;
			    grp.zTckDetailModel.Status = rec.SortId;
			}
        }
        ...
        grp.AssignedToOnChange = function() {
		  ...
		  if(grp.zModel.AssignedToVal)
            ...
            if (rec) {
			    ...
			    grp.zTckDetailModel.AssignedTo = rec.UserName;
        ...
        grp.ListTkts = function(cat) {
            ...
            zSrv_InputCustom.httpGet(getUrl, {}).then(function (respTcktsList) {
			    ...
			    grp.zSuppTicketsData = respTcktsList;
        ...
        grp.SerachTickets = function() { 
		  ...
		  console.log("Search Val : "+grp.zModel.SearchVal);
		  
		  zSrv_InputCustom.httpGet('/api/searchtckts/'+grp.zModel.SearchVal, {}).then(function (respTcktsList) { 
		...
		grp.newTktDetailModel = function() {
		 
		    grp.isLoading = true;
		   
		    grp.zTckDetailModel.TicketId = grp.zModel.Id;
		    
		  
		    zSrv_InputCustom.httpPost('/api/TicketDetailsFlows', grp.zTckDetailModel).then(function (respTckDetails) {
                   
			        console.log("Response Details : "+JSON.stringify(respTckDetails));
				    
			        grp.zData.TcktsDetails.push(respTckDetails);
        ...
        grp.getModelAfterSuccess = function() {
            ...
            if (grp.ngLocation.path().indexOf(grp.editModelURL) >= 0) {
			    ...
			    console.log("TicketDetailsFlows : "+JSON.stringify(grp.zModel.TicketDetailsFlows));
			  
			    /*
			    $("#accord_AddComment").click();				      
				console.log("accord clicked");
				*/
	    ...
	    grp.accSuppTicketClcik = function(Id){
		  
		  
		  /*$('#accord_AddComment').collapse('show');*/
		  console.log("Parent Support Ticket accord clicked : Id : "+Id);
		  
		  var ctrlID = "#SuppTicket"+Id;
		  
		  var AddNewCommand = "#accord_AddComment"+Id;
		  
		  $(ctrlID).collapse('show');
		  $(AddNewCommand).collapse('show');
        ...
        mg.onFileUploadSuccess = function () {
            ...
            var data = mg.successResult.data;
		     zSrv_InputCustom.httpGet('/api/getFilesBySupportId/'+grp.zModel.Id, {}).then(function (respFiles) {
				   ...
				   grp.zData.Files = respFiles;
			       console.log("Files : "+JSON.stringify(grp.zData.Files));
		...
		var fileDetailsMg = $scope.modalGroup;

        grp.newTckDetailsFileUpload = function (ev) {
            mg.modalHeader = 'Upload File';
            //zMV_eSupportFileUpload.html
            var view = "/Scripts/app/views/eSupport/zMV_eSupportFileUpload.html";
            fileDetailsMg.files = {};
            fileDetailsMg.showModal($scope, ev, view, false).then(function () {
            }, function () { });
        }

        fileDetailsMg.onFileUploadSuccess = function () {
            console.log("Inside the fileDetailsMg onFileUploadSuccess");
            var data = fileDetailsMg.successResult.data;
		    grp.zTckDetailFile.push(data);
		    console.log("File Data : "+JSON.stringify(data));
		   
		    grp.zTckDetailModel.files = grp.zTckDetailFile;
